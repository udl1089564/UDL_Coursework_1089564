...
def split_categorical_noDINO(num_epochs=100, batch_size=15000, coreset_size=None, beta=1):
    filename = 'split_categorical_noDINO'
    assert coreset_size.__class__ is list
    dataloaders = data.get_split_dataloaders(
        class_distribution, batch_size=batch_size)
    model = SplitModel()
    model.to(device)
...
if __name__=='__main__':
    ...
    batch_size = 256
    coreset_size = [0, 200]
    split_categorical_noDINO(num_epochs=num_epochs, batch_size=batch_size, coreset_size=coreset_size, beta=1)