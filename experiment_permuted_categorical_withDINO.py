...
def permuted_categorical_withDINO(num_epochs=100, batch_size=256, coreset_size=None, beta=1):
    filename = 'permuted_categorical_withDINO'
    assert coreset_size.__class__ is list
    dataloaders = data.get_permuted_dataloaders(num_tasks, batch_size, dino_transform=True)
    model = PermutedModel(starting_dim=384)
    model.to(device)
...
if __name__=='__main__':
    ...
    batch_size = 256
    coreset_size = [0, 200]
    permuted_categorical_withDINO(num_epochs=num_epochs, batch_size=batch_size, coreset_size=coreset_size, beta=1)